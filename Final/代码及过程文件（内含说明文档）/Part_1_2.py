...
parser.add_argument("--lr_d", type=float, default=0.00005, help="adam: learning rate")
...
# 2. 加载权重文件
generator_weights = torch.load("./models/generator_200.pth", map_location=device)
discriminator_weights = torch.load("./models/discriminator_200.pth", map_location=device)
...
def sample_image(n_row, batches_done):
    ...
    save_image(image, "./images/step1/results_opt1/%d.png" % batches_done, nrow=n_row, normalize=True)
...
def optimize_display(output_history, classes):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("./images/step1/optimize_opt1_display.png")
    # plt.show()
...
for epoch in tqdm(range(opt.n_epochs)):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        d_real_loss = adversarial_loss(validity_real, valid - 0.05)

        # Loss for fake images
        validity_fake = discriminator(gen_imgs.detach(), gen_labels)
        d_fake_loss = adversarial_loss(validity_fake, fake + 0.05)
    ...
    if (epoch >= 10
            and torch.mean(torch.std(loss_history[epoch-10:epoch, :], dim=1)) < 0.12
            and torch.std(torch.mean(loss_history[epoch-10:epoch, :], dim=1)) < 0.15
    ):
...
torch.save(generator.state_dict(), './models/generator_opt1.pth')
torch.save(discriminator.state_dict(), './models/discriminator_opt1.pth')