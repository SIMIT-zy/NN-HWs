...
class Generator(nn.Module):
    ...
    def forward(self, noise, labels):
        ...
        out = 0.6 * gen_labels + 0.4 * gen_noise
...
def sample_image(n_row, batches_done):
    ...
    save_image(image, "./images/step1/results_200/%d.png" % batches_done, nrow=n_row, normalize=True)
...
def optimize_display(output_history, classes):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("./images/step1/optimize_200_display.png")
    # plt.show()
...
for epoch in tqdm(range(opt.n_epochs)):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        g_steps = int(torch.ceil(torch.abs(g_loss - 1.0) * 10).clamp(1, 3))
        d_steps = int(torch.ceil(torch.abs(d_loss - 0.5) * 10).clamp(1, 3))
...
optimize_display(loss_history, ["g_loss", "d_real_loss", "d_fake_loss", "d_loss"])
torch.save(generator.state_dict(), './models/generator_200.pth')
torch.save(discriminator.state_dict(), './models/discriminator_200.pth')